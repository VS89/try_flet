...
class TeamWithCountScores(ft.UserControl):
    ...
    def __init__(self, home_team_flag=False, away_team_flag=False):
        super().__init__()
        self.txt_number_home = ft.TextField(value="0", width=50, height=40, read_only=True)
            ...
            return ft.Row(
                ...
                controls=[
                    ...
                    ft.Column(controls=[
                        ...
                        ft.Row(controls=[self.txt_number_home]),
    ...
    async def plus_click(self, e):
        if int(self.txt_number_home.value) <= 8:
            self.txt_number_home.value = str(int(self.txt_number_home.value) + 1)
        else:
    ...
    async def minus_click(self, e):
        if int(self.txt_number_home.value) >= 1:
            self.txt_number_home.value = str(int(self.txt_number_home.value) - 1)
        else:
